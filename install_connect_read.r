...
#_______________________________________________________________________________
  ...
  # 2.1 Corrigindo o tipo da variável V_VTCC_m3f
dadosInv <- dadosInv %>% mutate(V_VTCC_m3f = as.numeric(V_VTCC_m3f))
...
#_______________________________________________________________________________
  ...
  ' Particionar melhora o desempenho em queries filtradas
     ...
     filtra por Região ou Regime de Manejo, por exemplo'

  # 4.1 Particionar os dados (por Região e Regime de Manejo)
spark_write_parquet(dadosPqt,
                    path = "hdfs:///data/raw/inventario/particionado/",
                    partition_by = c("REGIAO", "REGIMEMANEJO"),
                    mode = "overwrite")

  # 4.2 Leia o Parquet particionado
dadosPart <- spark_read_parquet(sc,
                                name = "invpart",
                                path = "hdfs:///data/raw/inventario/particionado/")

  # 4.3 Visualizando novamente os dados na tela (pode usar um comando ou outro)
glimpse(dadosPart)
dadosPart %>% glimpse()
...
schema_info <- sdf_schema(dadosBrutos)
  ...
  # 5.2 Obtém a lista de colunas de variáveis numéricas
num_cols <- names(schema_info)[sapply(schema_info, 
                                      function(x) x$type %in% c("DoubleType", 
                                                                "IntegerType", 
                                                                "FloatType", 
                                                                "LongType"))]

  # 5.3 Obtém a lista de colunas de variáveis categóricas
cat_cols <- setdiff(colnames(dadosBrutos), num_cols)

  # 5.4 Remove colunas específicas do agrupamento (para evitar duplicação)
cat_cols <- setdiff(cat_cols, c("DATAATUALIZACAO", 
                                "DATAMEDICAO", 
                                "PARCELA",
                                "AREAPARCELAM2"))
...
dadosMedicoes <- dadosBrutos %>%
  ...
  # 5.6 Visualiza os dados e verifica a quantidade
